...
void GetInitialData( CalculationData^ data )
{
	...
	if( data->solution->weapon )
	{
		if( data->solution->abilities.ContainsKey( data->solution->weapon->ability ) )
			data->solution->abilities[ data->solution->weapon->ability ] += data->solution->weapon->ability_amount;
		else
			data->solution->abilities.Add( data->solution->weapon->ability, data->solution->weapon->ability_amount );

		data->solution->defence += data->solution->weapon->defence;
		data->solution->max_defence += data->solution->weapon->defence;
	}
...
void Solution::AddExtraSkills()
{
	...
	for each( Collections::Generic::KeyValuePair< Ability^, int >^ ap in abilities )
	{
		if( ap->Key != SpecificAbility::torso_inc )
		{
			...
			{
				nullptr,
				Decoration::GetBestDecoration( ap->Key, 1, data->query->hr, data->query->elder_star ),
				Decoration::GetBestDecoration( ap->Key, 2, data->query->hr, data->query->elder_star ),
				Decoration::GetBestDecoration( ap->Key, 3, data->query->hr, data->query->elder_star )
			};
