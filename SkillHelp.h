...
#pragma endregion
		...
		System::Void SelectInfoInternal( String^ name )
		{
			...
			else
			{
				...
				while( e.MoveNext() )
				{
					...
					txtDescription->AppendText( L".  " + e.Current.Value->description );
					txtDescription->AppendText( Environment::NewLine );
					...
					//txtDescription->SelectionFont = gcnew Drawing::Font( txtDescription->Font, FontStyle::Bold );
				}
			}
		...
		System::Void cmsDescription_Opening(System::Object^  sender, System::ComponentModel::CancelEventArgs^  e)
		{
			...
			if( line >= txtDescription->Lines->Length )
				return;
			String^ str = txtDescription->Lines[ line ];
			if( str == L"" )
				return;

			if( line >= armor_list_begin && line - armor_list_begin < armors.Count )
			{
